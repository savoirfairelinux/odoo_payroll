...
class TestPayslipBase(common.TransactionCase):
    ...
    def setUp(self):
        ...
        self.jurisdiction_id = self.data_model.get_object_reference(
            cr, uid, 'sfl_payroll_canada', 'jurisdiction_federal')[1]
        ...
        self.comp_activity_id = self.ref(
            'sfl_payroll_activity.activity_holiday_status_comp')

        self.public_activity_id = self.ref(
            'sfl_payroll_activity.activity_holiday_status_public')

        self.sl_activity_id = self.ref(
            'sfl_payroll_activity.activity_holiday_status_sl')

        self.unpaid_activity_id = self.ref(
            'sfl_payroll_canada.activity_holiday_status_vac_unpaid')
